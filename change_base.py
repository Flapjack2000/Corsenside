...
def change_base(num: int, base: int):
    ...
    upper_limit: int = max(digit_table.keys()) + 1

    if base < 2 or base > upper_limit:
        raise ValueError(f"Base must be between 2 and {upper_limit}.")
    ...
    while num != 0:
        ...
        remainder: str = digit_table[remainder] if remainder in digit_table else remainder
...
print(change_base(5472507324895732409532, 100))